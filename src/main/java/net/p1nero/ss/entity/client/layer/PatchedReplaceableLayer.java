...
@OnlyIn(Dist.CLIENT)
public class PatchedReplaceableLayer<E extends LivingEntity & OwnableEntity, T extends LivingEntityPatch<E>, M extends EntityModel<E>, AM extends AnimatedMesh> extends PatchedLayer<E, T, M, RenderLayer<E, M>, AM> {
    ...
    protected void renderLayer(T entityPatch, E entity, RenderLayer<E, M> vanillaLayer, PoseStack postStack, MultiBufferSource buffer, int packedLightIn, OpenMatrix4f[] poses, float bob, float yRot, float xRot, float partialTicks) {
        if(entityPatch.getArmature() instanceof IReplaceableArmature armature){
            ItemStack mainHandStack = entity.getOwner() instanceof LivingEntity owner ? owner.getMainHandItem() : EpicFightItems.IRON_LONGSWORD.get().getDefaultInstance();
     ...
     * 根据对应要替换Joint的位置渲染
     */
    public void renderScreenSword(ItemStack stack, LivingEntityPatch<?> entityPatch, IReplaceableArmature armature, OpenMatrix4f[] poses, MultiBufferSource buffer, PoseStack poseStack, int packedLight) {
        for(Joint joint : armature.getJoints()){
            OpenMatrix4f jointTransform = poses[joint.getId()];
//            jointTransform.removeTranslation(); TODO 验证是否有用
            poseStack.pushPose();
            this.mulPoseStack(poseStack, jointTransform);
